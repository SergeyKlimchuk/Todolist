...
// Изменения ярлыка
$('.tool-tag-changer li').on('click', function (event) {
    // Ищем чекбокс
    $radio = $(event.currentTarget).find('input:checkbox');

    // Проверка на модификацию сегмента передачи
    if ($radio.length == 0) {
        alert("LOGIC ERROR");
        return false;
    }
    $checkbox = $radio[0];
    // Берём значение чекбокса
    $check = !$checkbox.checked;
    $checkbox.checked = $check;
    // Берем Id задачи
    $taskId = $(event.currentTarget).parents('.task-box')[0].id;
    // Берём Id ярлыка
    $labelId = $checkbox.name;

    //console.log("labelId: " + $labelId + ", taskId" + $taskId);

    // Отправка AJAX-запроса
    $.ajax({
        url: "/Home/EditLabel",
        type: "GET",
        data: "taskId=" + $taskId + "&labelId=" + $labelId + "&actionId=" + $check,
        success: function (response) { }
    });
